...
void DrawFilledmoRectangleWithBorder(RGBABitmapImage *image, double x, double y, double w, double h, RGBA *borderColor, RGBA *fillColor){
  if(h > 0.0 && w > 0.0){
    DrawFilledmoRectangle(image, x, y, w, h, fillColor);
    DrawmoRectangle1px(image, x, y, w, h, borderColor);
  }
}
...
bool moRectanglesOverlap(moRectangle *r1, moRectangle *r2){
  bool overlap;
...
moRectangle *CreatemoRectangle(double x1, double y1, double x2, double y2){
  moRectangle *r;
  r = new moRectangle();
  r->x1 = x1;
  r->y1 = y1;
...
void CopymoRectangleValues(moRectangle *rd, moRectangle *rs){
  rd->x1 = rs->x1;
  rd->y1 = rs->y1;
  rd->x2 = rs->x2;
  rd->y2 = rs->y2;
}
void DrawXLabelsForPriority(double p, double xMin, double oy, double xMax, double xPixelMin, double xPixelMax, NumberReference *nextmoRectangle, RGBA *gridLabelColor, RGBABitmapImage *canvas, vector<double> *xGridPositions, StringArrayReference *xLabels, NumberArrayReference *xLabelPriorities, vector<moRectangle*> *occupied, bool textOnBottom){
  ...
  for(i = 0.0; i < xLabels->stringArray->size(); i = i + 1.0){
    if(xLabelPriorities->numberArray->at(i) == p){
      ...
      for(j = 0.0; j < nextmoRectangle->numberValue; j = j + 1.0){
        currentOverlaps = currentOverlaps || moRectanglesOverlap(r, occupied->at(j));
      }

      if( !currentOverlaps  && p == 1.0){
        DrawText(canvas, r->x1 + padding, r->y1 + padding, text, gridLabelColor);

        CopymoRectangleValues(occupied->at(nextmoRectangle->numberValue), r);
        nextmoRectangle->numberValue = nextmoRectangle->numberValue + 1.0;
      }
...
void DrawYLabelsForPriority(double p, double yMin, double ox, double yMax, double yPixelMin, double yPixelMax, NumberReference *nextmoRectangle, RGBA *gridLabelColor, RGBABitmapImage *canvas, vector<double> *yGridPositions, StringArrayReference *yLabels, NumberArrayReference *yLabelPriorities, vector<moRectangle*> *occupied, bool textOnLeft){
  bool overlap, currentOverlaps;
...
double MapXCoordinateBasedOnSettings(double x, ScatterPlotSettings *settings){
  ...
  moRectangle *boundaries;

  boundaries = new moRectangle();
...
bool DrawScatterPlotFromSettings(RGBABitmapImageReference *canvasReference, ScatterPlotSettings *settings, StringReference *errorMessage){
  ...
  NumberReference *nextmoRectangle, *x1Ref, *y1Ref, *x2Ref, *y2Ref, *patternOffset;
  ...
  if(success){
    ...
    DrawmoRectangle1px(canvas, xPixelMin, yPixelMin, xLengthPixels, yLengthPixels, settings->gridColor);
...
if(settings->yAxisLeft){
    ...
    /* Labels */
    occupied = new vector<moRectangle*> (xLabels->stringArray->size() + yLabels->stringArray->size());
    for(i = 0.0; i < occupied->size(); i = i + 1.0){
      occupied->at(i) = CreatemoRectangle(0.0, 0.0, 0.0, 0.0);
    }
    nextmoRectangle = CreateNumberReference(0.0);
    ...
    for(i = 1.0; i <= 5.0; i = i + 1.0){
      ...
      DrawXLabelsForPriority(i, xMin, originYPixels, xMax, xPixelMin, xPixelMax, nextmoRectangle, gridLabelColor, canvas, xGridPositions, xLabels, xLabelPriorities, occupied, textOnBottom);
    }
    ...
    for(i = 1.0; i <= 5.0; i = i + 1.0){
      ...
      DrawYLabelsForPriority(i, yMin, originXPixels, yMax, yPixelMin, yPixelMax, nextmoRectangle, gridLabelColor, canvas, yGridPositions, yLabels, yLabelPriorities, occupied, textOnLeft);
    }
...
void ComputeBoundariesBasedOnSettings(ScatterPlotSettings *settings, moRectangle *boundaries){
  ScatterPlotSeries *sp;
...
bool DrawBarPlotFromSettings(RGBABitmapImageReference *canvasReference, BarPlotSettings *settings, StringReference *errorMessage){
  ...
  vector<moRectangle*> *occupied;
  NumberReference *nextmoRectangle;
  ...
  if(success){
    ...
    /* Labels */
    occupied = new vector<moRectangle*> (yLabels->stringArray->size());
    for(i = 0.0; i < occupied->size(); i = i + 1.0){
      occupied->at(i) = CreatemoRectangle(0.0, 0.0, 0.0, 0.0);
    }
    nextmoRectangle = CreateNumberReference(0.0);
    ...
    for(i = 1.0; i <= 5.0; i = i + 1.0){
      DrawYLabelsForPriority(i, yMin, xPixelMin, yMax, yPixelMin, yPixelMax, nextmoRectangle, gridLabelColor, canvas, yGridPositions, yLabels, yLabelPriorities, occupied, true);
    }
    ...
    for(n = 0.0; n < bs; n = n + 1.0){
      for(s = 0.0; s < ss; s = s + 1.0){
        ...
        if(settings->barBorder){
          DrawFilledmoRectangleWithBorder(canvas, Round(x), Round(y), Round(w), Round(h), GetBlack(), barColor);
        }else{
          DrawFilledmoRectangle(canvas, Round(x), Round(y), Round(w), Round(h), barColor);
        }
...
void DrawmoRectangle1px(RGBABitmapImage *image, double x, double y, double width, double height, RGBA *color){
...
void DrawFilledmoRectangle(RGBABitmapImage *image, double x, double y, double w, double h, RGBA *color){
  double i, j;
...
void DrawLineBresenhamsAlgorithmThick(RGBABitmapImage *canvas, double x1, double y1, double x2, double y2, double thickness, RGBA *color){
  ...
  if(thickness >= 3.0){
    ...
    DrawFilledmoRectangle(canvas, x, y, 2.0, 2.0, color);
